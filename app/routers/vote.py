...
def vote(vote: schemas.Vote,db:Session = Depends(get_db),
    ...
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail= f"Post with id: {vote.post_id} doesn't exits")
    ...
    if vote.dir ==1:
       ...
       db.add(new_vote)
       db.commit()
       return {"successs": "successfully added vote"}
        
    else:
        if not found_vote:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f" Vote does'nt exist")
        
        vote_query.delete(synchronize_session=False)
        db.commit()

        return {"success": "successfully deleted vote"}
